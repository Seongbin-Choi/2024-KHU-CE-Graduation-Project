# -*- coding: utf-8 -*-
"""졸프_10_31(환율 포함)_G

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1TxL3h315OusoyFHhxGjKSBcOUHhcIEw3
"""
...
# 환율 데이터의 결측치를 이전 값으로 대체 (앞쪽 값을 사용하여 보간)
df_fx_common.fillna(method='ffill', inplace=True)
...
# 배열 길이 맞추기
min_length = min(len(scaled_ss), len(scaled_ks), len(scaled_fx))
scaled_ss = scaled_ss[:min_length]
scaled_ks = scaled_ks[:min_length]
scaled_fx = scaled_fx[:min_length]

# 데이터를 병합 (삼성전자, KOSPI, 환율 데이터)
time_step = 10
combined_data = []
...
while current_date <= datetime.strptime(end_date, '%Y-%m-%d'):
    ...
    try:
        ...
        # 필요한 데이터가 충분한지 확인
        if len(ss_close) < time_step or len(ks_close) < time_step or len(fx_close) < time_step:
            print(f"Skipping {current_date}: Insufficient data points for scaling.")
        ...
        # 예측
        predicted_data_scaled = model.predict(X_test, verbose=0)
        ...
        print(f'Prediction for {current_date}: {prediction}')
